...
def segmentLP(imagePath, gui):
    ...
    for i, ctr in enumerate(sorted_ctrs):
        ...
        # Getting ROI
        roi = dst[y:y + h, x:x + w]
        cv2.imwrite('tmp/' + str(part) + '.png', roi)
        rois.append('tmp/' + str(part) + '.png')
        ...
        part += 1
    if len(rois) == 0:
        print("FILENAME: ", imagePath, " FAILED")
        return -1
    dst = cv2.resize(dst, (720, 160))
    dst = cv2.cvtColor(dst, cv2.COLOR_BGR2RGB)
    displayImg = Image.fromarray(dst)
    displayImg = ImageTk.PhotoImage(displayImg)
    #gui.showImage(displayImg)
    gui.showImage(displayImg)
